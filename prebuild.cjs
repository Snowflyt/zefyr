...
const renameImport = (code, oldName, newName = '') => {
  ...
  for (const importDeclaration of importDeclarations) {
    ...
    for (const namedImport of namedImports) {
      ...
      if (namedImport.getAliasNode() && namedImport.getAliasNode()?.getText() === oldName) {
        importPath = importDeclaration.getModuleSpecifierValue();
  ...
  sourceFile.forEachDescendant((node) => {
    if (node.getKindName() === 'ShorthandPropertyAssignment' && node.getText() === oldName)
...
const stringifyImport = (options) => {
  ...
  if (defaultSymbol) {
    ...
    result +=
      '{ ' +
      nonDefaultSymbols.map((s) => (typeof s === 'string' ? s : `${s[0]} as ${s[1]}`)).join(', ') +
      ' } ';
  }
  result += `from '${path}';`;
  return result;
  ...
  for (; i < lines.length; i++) {
    ...
    if (line.startsWith('import')) {
      ...
      const oldResolvedSymbols = [
        ...(imports[resolved.path]?.value.default ? [imports[resolved.path].value.default] : []),
        ...(imports[resolved.path]?.value.nonDefault.map((s) =>
          typeof s === 'string' ? s : s[1],
        ) ?? []),
        ...(imports[resolved.path]?.type.default ? [imports[resolved.path].type.default] : []),
      ...
      const resolvedSymbols = [
        ...
        ...(resolved.symbols.nonDefault.map((s) => (typeof s === 'string' ? s : s[1])) ?? []),
      ];
    ...
    for (const resolved of resolves) {
      ...
      if (resolved.path in imports) {
        ...
        const oldResolvedSymbolsValue = [
          ...
          ...(oldResolved.value.nonDefault.map((s) => (typeof s === 'string' ? s : s[1])) ?? []),
        ];
        const oldResolvedSymbolsType = [
          ...(oldResolved.type.default ? [oldResolved.type.default] : []),
          ...(oldResolved.type.nonDefault.map((s) => (typeof s === 'string' ? s : s[1])) ?? []),
        ];
        const oldResolvedSymbols = new Set([...oldResolvedSymbolsValue, ...oldResolvedSymbolsType]);
        for (const symbol of resolvedSymbols) {
          if (!oldResolvedSymbols.has(symbol)) {
            if (symbol === resolved.symbols.default) oldResolved[resolved.type].default = symbol;
            ...
            if (oldResolved.type.default === symbol) {
              ...
              oldResolved.type.nonDefault = oldResolved.type.nonDefault.filter((s) => s !== symbol);
...
for (const module of modules) {
  ...
  for (const filename of filenames) {
    ...
    const content = renameDuplicateImports(fs.readFileSync(filePathname, 'utf-8'));
...
for (const [path, { type: typeSymbols, value: valueSymbols }] of Object.entries(imports)) {
  if (path.startsWith('.')) {
...
for (const [, { type: typeSymbols, value: valueSymbols }] of Object.entries(imports)) {
  ...
  typeExports.push(...typeSymbols.nonDefault.map((s) => (typeof s === 'string' ? s : s[1])));
}
valueExports.sort();
typeExports.sort();
...
const result = [
  ...
  ...(valueExports.length > 0 ? [`export { ${valueExports.join(', ')} };`] : []),
  ...(typeExports.length > 0 ? [`export type { ${typeExports.join(', ')} };`] : []),
