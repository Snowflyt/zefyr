...
const median = <T>(array: T[] | readonly T[]): Median<typeof array> => {
  ...
  if (array.length === 0)
    throw new TypeError('Cannot get the median of an empty array');
  ...
  const sorted = Array.from(array).sort((a, b) => {
    ...
    if (typeof a !== type || typeof b !== type)
      throw new TypeError(
        'Cannot get the median of an array of mixed numbers and bigints',
      );
  ...
  if (sorted.length % 2 === 0)
    return ((((sorted[middle - 1] as number) +
      (sorted[middle] as number)) as R) /
